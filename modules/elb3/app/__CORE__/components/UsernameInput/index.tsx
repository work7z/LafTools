...
export default (props: { checkDuplicate?: boolean, name: string }) => {
    ...
    let [duplicate, setDuplicate] = React.useState(false)
    ...
    return (
        <div className=''>
            ...
            <div className="text-right text-xs text-gray-400 mt-2" id="hs-input-helper-text">{Dot("dqwq2", "User id is consisted of alphanumeric characters and should be unique.")}
                {
                    props.checkDuplicate && value != '' ? !onlyAlphValue ? <div className='text-red-500'>{Dot("-mwFO_9", "Contain invalid characters. We only accept alphanumeric characters!")}</div> : !duplicate ? <div className='text-green-500'>[{Dot("YZdRNcuy", "Congratulations! It is not yet used.")}]</div> : <div className='text-red-500'>[{Dot("RI3DtugPB", "Sorry, it is used already.")}]</div> : ''
                }
            </div>
        </div>
    )
